define(["modules/store","modules/builder"],function(storage,builder){
  var addNotes = document.getElementById("addNotes"),
      ...
      hideOrShowAll = document.getElementById("showAll");
  ...
  function showAll() {
    ...
    hideOrShowAll.textContent="Hide All";
    hideOrShowAll.id="hideAll";
  }
  function hideAll() {
    var idArray = storage.every();
    for (var i=0;i<idArray.length;i++) {
      if(document.getElementById(idArray[i])) hide(idArray[i]);
    }
    hideOrShowAll.textContent="Display All";
    hideOrShowAll.id="showAll";
  }
  function sidebar() {
